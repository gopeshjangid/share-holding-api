    ...
    Event.aggregate([
        ...
        .then(async (event) => {
            ...
            if (event) {
                ...
                jsonResult = utils.getJsonResponse(true, 'Event details.', inventoryDetails);
            } else {
                jsonResult = utils.getJsonResponse(false, 'Event detail not found.', null);
            }
            ...
            bb.on('field', async (field, val, info) => {
                ...
                if (field == '_id') {
                    ...
                    if (isEventIdExists == null) {
                        let jsonResult = utils.getJsonResponse(false, 'Event Id Not exists.', null);
...
const uploadImages = async (req, res, next) => {
    try {
        ...
        if (Array.isArray(req?.body?.eventImages)) {
            ...
            Event.findOneAndUpdate({ _id: ObjectId(req?.body?._id) }, { $set: set }, { new: true })
                .then((savedEvent) => {
                    let jsonResult = utils.getJsonResponse(true, 'Event updated successfully.', savedEvent);
                    res.send(jsonResult);
                })
                .catch((err) => {
                    if (err) {
...
const create = async (req, res, next) => {
    try {
        ...
        ticketDetails.sellingStartDateTime = new Date(ticketDetails.sellingStartDateTime);
        ticketDetails.sellingEndDateTime = new Date(ticketDetails.sellingEndDateTime);
        ...
        if (isEventExists != null) {
            let jsonResult = utils.getJsonResponse(false, 'Event already exists.', null);
            return res.send(jsonResult);
        } else {
            event
                .save()
                .then(async (savedEvent) => {
                    let jsonResult = utils.getJsonResponse(true, 'Event added successfully.', savedEvent);
...
const update = async (req, res, next) => {
    try {
        ...
        if (eventId) {
            ...
            Event.findOneAndUpdate({ _id: ObjectId(eventId) }, { $set: set }, { new: true })
                .then((savedEvent) => {
                    let jsonResult = utils.getJsonResponse(true, 'Event updated successfully.', savedEvent);
                    res.send(jsonResult);
                })
...
const list = async (req, res, next) => {
    ...
    if (req?.query?.time) {
        condition = {
            $or: [{ eventStartDateTime: new Date(req?.query?.time) }, { eventEndDateTime: new Date(req?.query?.time) }]
        };
    }
...
const createPaymentIntent = async (req, res, next) => {
    try {
        ...
        let jsonResult = utils.getJsonResponse(true, `Event Booked Successfully`, paymentIntent);
        res.send(jsonResult);
    } catch (err) {
        let jsonResult = utils.getJsonResponse(false, err, {});
        res.send(jsonResult);
...
const updateBookingVerifiedStatus = async (req, res, next) => {
    try {
        ...
        if (!isBookingIdExists) {
            let jsonResult = utils.getJsonResponse(false, 'Booking Id Not exists.', null);
            return res.send(jsonResult);
        }
        condition = { _id: ObjectId(bookingId) };
        let set = { isVerified: req?.body?.isVerified };

        Booking.findOneAndUpdate({ _id: ObjectId(bookingId) }, { $set: set }, { new: true }).then(
            async (savedEvent) => {
                let jsonResult = utils.getJsonResponse(true, 'Booking updated successfully.', savedEvent);
                res.send(jsonResult);
            }
        );
    } catch (err) {
...
const updateBooking = async (req, res, next) => {
    try {
        ...
        if (bookingId) {
            ...
            Booking.findOneAndUpdate({ _id: ObjectId(bookingId) }, { $set: set }, { new: true })
                .then(async (savedEvent) => {
                    let jsonResult = utils.getJsonResponse(true, 'Booking updated successfully.', savedEvent);
                    try {
            ...
            let jsonResult = utils.getJsonResponse(false, 'Please send bookingId ', {});
            res.send(jsonResult);
        }
    } catch (err) {
        let jsonResult = utils.getJsonResponse(false, err, {});
    ...
    try {
        ...
        if (isEventExists == null) {
            ...
            booking
                .save()
                .then((savedBooking) => {
                    let jsonResult = utils.getJsonResponse(true, 'Event booked successfully.', savedBooking);
                    res.send(jsonResult);
                })
                .catch((err) => {
                    if (err) {
...
const dashboard = async (req, res, next) => {
    try {
        ...
        let jsonResult = utils.getJsonResponse(true, 'Events dashboard.', {
            ...
            registrationPaid: clubMembersData.length > 0 ? clubMembersData[0]?.isPaid : false,
            netSale: 1010,
            recentSale: 209
        });
        res.send(jsonResult);
...
const adminBookingList = async (req, res, next) => {
    try {
        const { limit = 3, skip = 0, paymentStatus, bookingStatus, paymentId, stripeCustomerId } = req?.query;
        let condition = {};
        if (bookingStatus) {
            ...
            .then(async (bookings) => {
                ...
                if (bookings) {
                    let list = bookings.map(async (booking) => {
                        const club = await User.findOne({ _id: ObjectId(booking.Events?.userId) }, { name: 1 });
                        return {
                            ...booking,
                            clubName: club?.name
                        };
                    ...
                    jsonResult = utils.getJsonResponse(false, 'Booking list not found.', null);
                }
                res.send(jsonResult);
            })
            .catch((e) => next(e));
...
const registerClubUser = async (req, res, next) => {
    ...
    if (isEmailExists != null) {
        let jsonResult = utils.getJsonResponse(false, 'Email already exists.', null);
        res.send(jsonResult);
    } else {
        membership
            .save()
            .then((savedMembership) => {
                let jsonResult = utils.getJsonResponse(true, 'Club Member registered successfully.', savedMembership);
                res.send(jsonResult);
            })
            .catch((err) => {
                if (err) {
...
const createRegisterFormClubUser = async (req, res, next) => {
    ...
    if (isMembershipUrl != null) {
        let jsonResult = utils.getJsonResponse(false, 'Registration form already exists.', null);
        res.send(jsonResult);
    } else {
        membershipForm
            .save()
            .then(() => {
                let jsonResult = utils.getJsonResponse(true, 'Registration form created successfully.', null);
                res.send(jsonResult);
            })
            .catch((err) => {
                if (err) {
    ...
    try {
        ...
        if (formId) {
            ...
            if (isFormExists) {
                ...
                let jsonResult = utils.getJsonResponse(false, 'Membership form Not exists.', null);
                return res.send(jsonResult);
            }
            ...
            MembershipForm.findOneAndUpdate({ _id: ObjectId(formId) }, { $set: set }, { new: false })
                .then(() => {
                    let jsonResult = utils.getJsonResponse(true, 'Membership form updated successfully.', null);
                    res.send(jsonResult);
                })
                .catch((err) => {
                    if (err) {
...
const getRegisterFromData = (req, res) => {
    ...
    MembershipForm.findOne({
        ...
        .then((event) => {
            let jsonResult;
            if (event) {
                jsonResult = utils.getJsonResponse(true, 'Registration form Data', event);
            } else {
                jsonResult = utils.getJsonResponse(false, 'Registration form detail not found.', null);
            }
            return res.json(jsonResult);
        })
        .catch((e) => next(e));
            ...
            await Membership.aggregate([
                ...
                .then((clubMemberData) => {
                    ...
                    if (clubMemberData) {
                        ...
                        jsonResult = utils.getJsonResponse(true, `Club Members Data`, clubMemberData);
                    } else {
                        jsonResult = utils.getJsonResponse(false, 'Club Members list not found.', null);
                    }
                    res.send(jsonResult);
                })
                .catch((e) => next(e));
...
const createMembershipPaymentIntent = async (req, res, next) => {
    try {
        ...
        let jsonResult = utils.getJsonResponse(true, `Membership Booked Successfully`, paymentIntent);
        res.send(jsonResult);
    } catch (err) {
        let jsonResult = utils.getJsonResponse(false, err, {});
        res.send(jsonResult);
...
const updateMembershipBooking = async (req, res, next) => {
    try {
        ...
        if (bookingId) {
            ...
            Membership.findOneAndUpdate({ _id: ObjectId(bookingId) }, { $set: set }, { new: true })
                .then((data) => {
                    let jsonResult = utils.getJsonResponse(true, 'Booking updated successfully.', data);
                    res.send(jsonResult);
                })
                .catch((err) => {
                    if (err) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            ticketDetails = ticketDetails.map((ticket) => {
                const getBookedTicket = data?.ticketDetail.find((item) => item.ticketName === ticket?.ticketName);
                if (getBookedTicket) {
                    ticket.ticketLimit =
                        ticket.ticketLimit >= getBookedTicket?.count ? ticket.ticketLimit - getBookedTicket?.count : 0;
                }
            ...
            Inventory.findOneAndUpdate({ eventId: ObjectId(data?.eventId) }, { $set: set }, { new: true })
                .then(async (savedEvent) => {
                    resolve(savedEvent);
                })
                .catch((err) => {
...
const sendBookingEmail = (data) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const htmlToSend = await ejs.renderFile(path.join(__dirname, '../../../../eventBooking.ejs'), mailBody);
...
const clubBookings = async (req, res, next) => {
    ...
    try {
        ...
        jsonResult = utils.getJsonResponse(false, 'Events booking history list.', final);
        res.send(jsonResult);
    } catch (e) {
        jsonResult = utils.getJsonResponse(false, 'Events list not found.', null);
        res.send(jsonResult);
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < clubs.length; i++) {
                ...
                forms.isMember = await checkIfStudentMembershipPurchase(student?._id, _id);

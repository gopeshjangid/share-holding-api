...
async function create(req, res, next) {
    ...
    if (isEmailExists != null) {
        jsonResult = await utils.getJsonResponse(false, 'Email already exists', null);
        res.status(500).json(jsonResult);
    } else if (studentId != null) {
        jsonResult = await utils.getJsonResponse(false, 'Student Id already exists', null);
        res.status(500).json(jsonResult);
    } else {
        user.save()
            .then(async (savedUser) => {
                ...
                const htmlToSend = await ejs.renderFile(path.join(__dirname, '../../../../signup.ejs'), {
                ...
                jsonResult = await utils.getJsonResponse(true, 'Sign up has been completed', savedUser);
                ...
                if (err) {
                    if (err.name == 'ValidationError') {
                        for (field in err.errors) {
                            let jsonResult = await utils.getJsonResponse(false, err.errors[field].message, null);
                            jsonResult = await utils.getJsonResponse(true, err.errors[field].message, user);
                            res.status(500).send(jsonResult);
                        }
                    }
                } else {
                    jsonResult = await utils.getJsonResponse(false, 'Error in signup', null);
                    res.status(500).json(jsonResult);
                }
            });
    }
...
async function userLogin(req, res, next) {
    ...
    if (isUsernameExists == null) {
        let jsonResult = await utils.getJsonResponse(false, 'Email does not exists.', null);
        res.send(jsonResult);
    } else if (isUsernameExists && isWrongPassword == null) {
        let jsonResult = await utils.getJsonResponse(false, 'Incorrect Password.', null);
        res.status(500).send(jsonResult);
    } else {
        let condition = {
            email: req?.body?.email,
        ...
        User.findOne(condition)
            .then(async (user) => {
                ...
                let jsonResult;
                if (user) {
                    user.accessToken = await jwttoken.createToken(user?._id, user?.password, user?.email);
                    jsonResult = await utils.getJsonResponse(true, 'Student logged in successfully.', user);
                } else {
                    jsonResult = await utils.getJsonResponse(false, 'Student not found.', null);
                }
    ...
    User.get(userId)
        .then(async (user) => {
            ...
            if (user) {
                ...
                jsonResult = await utils.getJsonResponse(false, 'User detail not found.', null);
            }
...
async function sendPassCode(req, res, next) {
    ...
    OTP.findOneAndUpdate({ email: email }, { $set: set }, { upsert: true, new: true })
        .then(async (updatedOTP) => {
            let jsonResult = await utils.getJsonResponse(true, 'Verification code sent.', updatedOTP);
            const htmlToSend = await ejs.renderFile(path.join(__dirname, '../../../../passwordCode.ejs'), {
                code: otp
            });
            await utils.sendEmail({
                email: email,
    ...
    try {
        ...
        if (userId) {
            ...
            if (isUserIdExists) {
                ...
                let jsonResult = await utils.getJsonResponse(false, 'User Id Not exists.', null);
            ...
            User.findOneAndUpdate({ _id: ObjectId(userId) }, { $set: set }, { new: true })
                .then(async (savedUser) => {
                    let jsonResult = await utils.getJsonResponse(true, 'User updated successfully.', savedUser);
                    res.send(jsonResult);
                })
                .catch(async (err) => {
                    if (err) {
            ...
            let jsonResult = await utils.getJsonResponse(false, 'Please send userId ', {});
            res.send(jsonResult);
        }
    } catch (err) {
    ...
    User.list({ limit, skip })
        .then(async (users) => {
            ...
            if (users) {
                ...
                jsonResult = await utils.getJsonResponse(false, 'Users list not found.', null);
            }
            res.send(jsonResult);
        })
        .catch((e) => next(e));
...
async function resetPassword(req, res) {
    ...
    if (isEmailExists === null) {
        res.status(500).send(utils.getJsonResponse(false, 'Email does not exists.', null));
    } else {
        const set = {
            password: req?.body?.password
        };
        User.findOneAndUpdate({ email: req?.body?.email }, { $set: set }, { new: false })
            .then(async (savedUser) => {
                let jsonResult = await utils.getJsonResponse(true, 'Password updated successfully.', savedUser);
                res.send(jsonResult);
            })
            .catch(async (e) => {
...
const getStudentBookings = async (req, res, next) => {
    ...
    try {
        ...
        jsonResult = utils.getJsonResponse(false, 'Error in fetching bookings.', null);
        res.send(jsonResult);
    }
};

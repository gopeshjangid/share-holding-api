...
async function companyLogin(req, res, next) {
	...
	if (password === "") {
		return res.send(
			utils.getJsonResponse(false, "Password is required.", null)
		);
	}
	Company.findOne({ pan, password })
		.then(async (user) => {
			...
			let jsonResult;
			if (user) {
				user.token = jwtToken.createToken(
					user?._id,
					user?.email,
					user.password
				);
...
async function registration(req, res, next) {
	...
	if (valid.length) {
		return res.send(
			utils.getJsonResponse(false, `${valid.join(", ")} required`, null)
		);
	}
